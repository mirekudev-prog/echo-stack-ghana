...
async def debug_db(db: Session = Depends(get_db)):
    ...
    masked_url = db_url[:30] + "..." if db_url != "not set" and len(db_url) > 30 else db_url
    ...
    try:
        ...
        recent_posts = db.query(models.Post).order_by(models.Post.created_at.desc()).limit(3).all()
        recent = [{"id": p.id, "title": p.title, "status": p.status} for p in recent_posts]
...
async def admin_reset_password(uid: str, request: Request, new_password: str = Form(...), db: Session = Depends(get_db)):
    ...
    u.email_verified = True
    db.commit()
    return {"success": True, "message": f"Password reset for {u.username}. User is now verified."}
...
async def toggle_bookmark(post_id: int, request: Request, db: Session = Depends(get_db)):
...
async def get_notifications(request: Request, limit: int = 50, db: Session = Depends(get_db)):
...
def create_notification(user_id, notif_type: str, title: str, message: str, 
                       link: str = "", actor_id=None, actor_username="", db=None):
...
@app.get("/api/search")
async def search(
    request: Request, q: str = "", content_type: str = "", 
    region_id: str = "", sort: str = "recent", limit: int = 20, offset: int = 0,
    db: Session = Depends(get_db)
):
...
async def get_trending(limit: int = 10, db: Session = Depends(get_db)):
    ...
    try:
        posts = db.query(models.Post).filter(
            ...
            (models.Post.likes + models.Post.views / 10).desc()
        ).limit(limit).all()
...
async def get_recommendations(request: Request, limit: int = 10, db: Session = Depends(get_db)):
    """Get personalized recommendations based on followed users and topics."""
    sid = request.cookies.get("user_session")
    try:
        if sid:
            followed = db.query(models.Follow.following_id).filter(
                models.Follow.follower_id == sid
            ).all()
            ...
            if following_ids:
                ...
                posts = db.query(models.Post).filter(
                    models.Post.status == "published"
                ).order_by(models.Post.likes.desc()).limit(limit).all()
        else:
            posts = db.query(models.Post).filter(
                models.Post.status == "published"
            ).order_by(models.Post.likes.desc()).limit(limit).all()
...
async def get_collections(request: Request, db: Session = Depends(get_db)):
    ...
    try:
        collections = db.query(models.Collection).filter(
            models.Collection.user_id == sid
        ).order_by(models.Collection.updated_at.desc()).all()
        
        result = []
        for c in collections:
            item_count = db.query(models.CollectionItem).filter(
...
async def create_collection(
    ...
    cover_image: str = Form(""), is_public: str = Form("1"),
    db: Session = Depends(get_db)
):
...
async def get_collection(collection_id: int, db: Session = Depends(get_db)):
    ...
    try:
        ...
        ).order_by(models.CollectionItem.display_order).all()
...
async def add_to_collection(collection_id: int, post_id: int, 
...
async def remove_from_collection(collection_id: int, post_id: int,
                                  request: Request, db: Session = Depends(get_db)):
...
async def get_user_settings(request: Request, db: Session = Depends(get_db)):
    ...
    try:
        settings = db.query(models.UserSetting).filter(
            models.UserSetting.user_id == sid
        ).first()
        ...
        return {"dark_mode": False, "language": "en", "email_notifications": True,
                "push_notifications": True, "show_follower_count": True, "show_post_stats": True}

@app.put("/api/settings")
async def update_user_settings(
    request: Request,
    dark_mode: str = Form(None), language: str = Form(None),
    email_notifications: str = Form(None), push_notifications: str = Form(None),
    show_follower_count: str = Form(None), show_post_stats: str = Form(None),
    db: Session = Depends(get_db)
):
    ...
    try:
        ...
        if email_notifications is not None:
            settings.email_notifications = 1 if email_notifications in ("1", "true", "True") else 0
        if push_notifications is not None:
            settings.push_notifications = 1 if push_notifications in ("1", "true", "True") else 0
        if show_follower_count is not None:
            settings.show_follower_count = 1 if show_follower_count in ("1", "true", "True") else 0
        if show_post_stats is not None:
            settings.show_post_stats = 1 if show_post_stats in ("1", "true", "True") else 0
...
async def get_messages(user_id: str, request: Request, limit: int = 50, db: Session = Depends(get_db)):
    ...
    try:
        messages = db.query(models.DirectMessage).filter(
            or_(
                and_(models.DirectMessage.sender_id == sid, models.DirectMessage.receiver_id == user_id),
                and_(models.DirectMessage.sender_id == user_id, models.DirectMessage.receiver_id == sid)
            )
...
async def send_message(user_id: str, request: Request, content: str = Form(...),
                      db: Session = Depends(get_db)):
...
async def send_tip(user_id: str, request: Request, amount: int = Form(100),
    ...
    try:
        ...
        return {"success": True, "tip_id": tip.id, "message": "Tip sent! (Payment integration pending)"}
    except HTTPException:
        raise
    except Exception as e:
        db.rollback()
...
async def subscribe_to_creator(creator_id: str, request: Request,
                               tier: str = Form("free"), db: Session = Depends(get_db)):
...
async def unsubscribe_from_creator(creator_id: str, request: Request,
                                  db: Session = Depends(get_db)):
...
async def get_my_subscriptions(request: Request, db: Session = Depends(get_db)):
    ...
    try:
        ...
        result = []
        for s in subs:
            creator = db.query(models.User).filter(models.User.id == s.creator_id).first()
            if creator:
                result.append({
...
async def get_badges(db: Session = Depends(get_db)):
    ...
    badge_types = [
        ...
        {"type": "ambassador", "name": "Cultural Ambassador", "icon": "fa-globe-africa"},
...
async def award_badge(user_id: str, request: Request, 
                     badge_type: str = Form(...), badge_name: str = Form(""),
                     badge_icon: str = Form(""), db: Session = Depends(get_db)):
...
async def share_post(post_id: int, request: Request, platform: str = Form("internal"),
                    db: Session = Depends(get_db)):
    """Record a share."""
    sid = request.cookies.get("user_session")
    try:
        share = models.Share(
            user_id=sid if sid else None,
            post_id=post_id,
            platform=platform
        )
        db.add(share)
...
async def get_following(username: str, limit: int = 50, db: Session = Depends(get_db)):
    ...
    try:
        ...
        result = []
        for f in follows:
            following = db.query(models.User).filter(models.User.id == f.following_id).first()
            if following:
                result.append({
                    "id": str(following.id),
...
# ─── LIKED POSTS ──────────────────────────────────────────────────────────────
@app.get("/api/likes")
async def get_liked_posts(request: Request, limit: int = 50, db: Session = Depends(get_db)):
    """Get posts liked by current user."""
    sid = request.cookies.get("user_session")
    if not sid:
        raise HTTPException(401, "Login required")
    try:
        likes = db.query(models.Comment).filter(
            models.Comment.user_id == sid,
            models.Comment.content == "__like__"
